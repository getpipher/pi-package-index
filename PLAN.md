...
- [x] **Phase 7 — Ship (separate turn)** — Vercel CLI deploy (personal, hobby) ✅ → CF CNAME `pi-package` → `cname.vercel-dns.com` (grey-cloud) ✅ → add prod domain + Let's Encrypt cert ✅ (`pi-package.rectorspace.com` live) → GitLab mirror `rz1989s/pi-package-index` + `GITLAB_SSH_KEY` + `MIRROR_ENABLED=true` ✅ (mirror run verified) → first full refresh dispatched ⏳ → **git connection in Vercel dashboard: RECTOR action pending** (enables push→deploy so the cron's data commits auto-deploy).